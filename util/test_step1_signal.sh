...
rm -Rf $OFDIR
mkdir -p $OFDIR
cmsRun $STPOL_DIR/runconfs/step1_newCmdLine_cfg.py inputFiles=$IN outputFile=$OFDIR/out_step1.root maxEvents=1000 doSkimming=False doSlimming=False &> $OFDIR/log_step1.txt
EX=$?
echo "Exit code:"$EX 
if [ "$EX" -ne 0 ]
then
