...
time cmsRun $STPOL_DIR/runconfs/step2_newCmdLine_preSelection_cfg.py doDebug=True inputFiles=$IN subChannel=T_t outputFile=$OFDIR/out.root maxEvents=10000 &> $OFDIR/log_step2.txt
EX=$?
echo "Exit code:"$EX 
if [ "$EX" -ne 0 ]
then
