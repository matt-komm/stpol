...
MultiCosThetaProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    for (const CosThetaCalculation& calculation: _calculations)
    {
        ...
        iEvent.getByLabel(calculation.restFrameInput,restFrameCollection);

        edm::Handle<edm::View<reco::Candidate>> particle1Collection;
        edm::Handle<edm::View<reco::Candidate>> particle2Collection;
        
        iEvent.getByLabel(calculation.particleInput1,particle1Collection);
        iEvent.getByLabel(calculation.particleInput2,particle2Collection);
        if (restFrameCollection->size()==0 || particle1Collection->size()==0 || particle2Collection->size()==0)
        {
        ...
        if (restFrameCollection->size()>1 || particle1Collection->size()>1 || particle2Collection->size()>1)
        {
            edm::LogWarning("MultiCosThetaProducer::produce()")<<"multiple particles for angle calculation found. Will only use the first ones!";
        }
